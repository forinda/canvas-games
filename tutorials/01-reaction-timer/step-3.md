...
```typescript
function readerHUD(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ctx.textAlign = 'left';
  ...
  if (state.bestAllTime > 0) {
    ...
    ctx.fillText(`🏆 All-time best: ${state.bestAllTime} ms`, width / 2, height - 30);
  }
}

function renderFinishScreen(ctx: CanvasRenderingContext2D, cx: number, cy: number) {
