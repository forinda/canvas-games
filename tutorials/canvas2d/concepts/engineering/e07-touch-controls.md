...
- `src/contexts/canvas2d/shared/TouchControls.ts` — The virtual control system
- `src/core/GameInterface.ts` — `TouchLayout` type and `GameDefinition.touchLayout`
- `src/platform/GameRegistry.ts` — Each game declares its `touchLayout`
- `src/contexts/canvas2d/games/snake/` — Reference implementation (engine + input system)
