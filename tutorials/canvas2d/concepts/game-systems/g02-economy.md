...
- **Tower Defense**: Gold earned from killing enemies is spent on placing and upgrading towers. The `EconomySystem` in `src/contexts/canvas2d/games/tower-defense/systems/EconomySystem.ts` tracks balance and validates purchases.
- **City Builder**: Revenue from taxes and buildings funds new construction. The `EconomySystem` in `src/contexts/canvas2d/games/city-builder/systems/EconomySystem.ts` handles income/expense flows.
