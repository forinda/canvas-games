...
- **Tower Defense**: During development, changing a tower's stats in `src/games/tower-defense/data/towers.ts` triggers HMR, and the change is visible in the browser without a full reload.
- **Platformer**: Level data edits in `src/games/platformer/data/levels.ts` are reflected instantly thanks to Vite's fast HMR.
