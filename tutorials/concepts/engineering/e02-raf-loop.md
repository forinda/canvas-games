...
- **All games**: Every game engine in this project uses `requestAnimationFrame` as the main loop driver. The engine classes (e.g., `src/games/tower-defense/game-engine.ts`, `src/games/platformer/PlatformerEngine.ts`) call rAF to schedule updates and renders.
