...
- **All games**: Every game in this project uses the attach/detach pattern through `PlatformAdapter` classes (e.g., `src/games/tower-defense/adapters/PlatformAdapter.ts`). The adapter adds input listeners on start and removes them on stop.
