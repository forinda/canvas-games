...
export class InputSystem implements InputHandler {
  ...
  private getShopRect(): { x: number; y: number; w: number; h: number } {
    const w = Math.max(SHOP_MIN_WIDTH, Math.min(SHOP_MAX_WIDTH, this.state.width * SHOP_WIDTH_RATIO));
  ...
  private onWheel(e: WheelEvent): void {
    ...
    const shop = this.getShopRect();
    if (mx >= shop.x) {
      const maxScroll = Math.max(0, this.state.upgrades.length * 72 - this.state.height + 60);
      this.state.shopScroll = Math.max(0, Math.min(maxScroll, this.state.shopScroll + e.deltaY));
    }
  }
}
