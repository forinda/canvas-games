...
export class HUDRenderer implements Renderable<IdleState> {
  render(ctx: CanvasRenderingContext2D, state: IdleState): void {
    ...
    ctx.fillText(`${formatNumber(state.cps)} per second`, cx, H * 0.45 + Math.min(gameW, H) * 0.15 + 50);
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';
    ctx.fillText(`Click power: ${formatNumber(state.clickPower)}`, statsX, statsY);
    ctx.fillText(`Total clicks: ${state.totalClicks.toLocaleString()}`, statsX, statsY + 18);
