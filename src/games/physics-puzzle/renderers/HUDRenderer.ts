...
export class HUDRenderer implements Renderable<PuzzleState> {
  render(ctx: CanvasRenderingContext2D, state: PuzzleState): void {
    ...
    // Overlays
    if (!state.started) {
      this.drawOverlay(ctx, 'PHYSICS PUZZLE', 'Click to place pieces, then SPACE to simulate!\nGuide the ball to the star.', '#f59e0b');
    } else if (state.solved) {
      this.drawOverlay(ctx, `LEVEL ${state.level} SOLVED!`, `Score: ${state.score}  |  Click for next level`, '#4ade80');
    }
  }
