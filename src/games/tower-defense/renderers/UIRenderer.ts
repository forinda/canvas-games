...
import { TOWER_DEFS, upgradeCost, sellRefund, getTowerStats } from '../data/towers';
...
export class UIRenderer {
  ...
  render(
    ...
    const btnLabel = isFirstWave ? '▶ Start Game' : canStart ? '▶ Next Wave' : '⏳ Wave...';
    ...
    ctx.fillStyle = '#444';
    ctx.textAlign = 'center';
    ctx.fillText('[P] Pause  [ESC] Deselect', btnX + btnW / 2, btnY + btnH + 14);
  ...
  private renderUpgradePanel(
    ...
    ctx.textAlign = 'right';
    ctx.fillText('★'.repeat(tower.level) + '☆'.repeat(3 - tower.level), panelX + panelW - 14, panelY2 + 14);
