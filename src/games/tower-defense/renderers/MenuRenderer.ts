...
export class MenuRenderer {
  ...
  private renderMainMenu(
    ...
    ctx.fillText('Defend your base – place towers, survive the waves!', w / 2, h * 0.37);

    // Mode buttons
    const modes: { label: string; key: string; desc: string; color: string }[] = [
      { label: 'CLASSIC', key: 'classic', desc: '10 waves · 20 lives · 200 gold', color: '#2ecc71' },
      { label: 'ENDLESS', key: 'endless', desc: 'Infinite waves · 15 lives · 150 gold', color: '#e67e22' },
      { label: 'CHALLENGE', key: 'challenge', desc: '10 waves · 10 lives · 100 gold', color: '#e74c3c' },
    ];
    ...
    modes.forEach((mode, i) => {
      ...
      this.drawMenuButton(ctx, bx, btnY, btnW, btnH, mode.label, mode.desc, mode.color);
      input.menuButtonRects.push({ label: mode.key, x: bx, y: btnY, w: btnW, h: btnH });
    });
    ...
    input.menuButtonRects.push({ label: 'exit', x: exitX, y: exitY, w: exitW, h: exitH });

    // Controls help
    ctx.font = `${Math.min(13, w * 0.018)}px monospace`;
    ctx.fillStyle = '#2a4a2a';
    ctx.textAlign = 'center';
    ctx.fillText('Click a cell to place a tower · [P] Pause · [ESC] Deselect · [Space] Start Wave', w / 2, h * 0.85);
  }
  ...
  private renderPauseMenu(
    ...
    input.menuButtonRects.push({ label: 'resume', x: bx1, y: by, w: btnW2, h: btnH2 });

    this.drawMenuButton(ctx, bx2, by, btnW2, btnH2, 'MAIN MENU', '', '#e67e22');
    input.menuButtonRects.push({ label: 'menu', x: bx2, y: by, w: btnW2, h: btnH2 });
    ...
    input.menuButtonRects.push({ label: 'exit', x: exitBx, y: exitBy, w: exitW2, h: btnH2 });
  ...
  private renderGameOver(
    ...
    ctx.fillStyle = '#aaa';
    ctx.fillText(`Score: ${state.score}   Wave: ${state.currentWave}`, w / 2, h * 0.45);
    ...
    input.menuButtonRects.push({ label: 'restart', x: bx1, y: by2, w: bw, h: bh });
  ...
  private renderWin(
    ...
    ctx.textAlign = 'center';
    ctx.font = `bold ${Math.min(72, w * 0.10)}px monospace`;
    ctx.fillStyle = '#2ecc71';
    ctx.shadowColor = '#2ecc71';
    ctx.shadowBlur = 30;
    ctx.textBaseline = 'middle';
    ...
    input.menuButtonRects.push({ label: 'restart', x: bx1, y: by3, w: bw, h: bh });
