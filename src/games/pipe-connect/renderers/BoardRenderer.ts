...
export class BoardRenderer implements Renderable<PipeState> {
  ...
  private drawCell(
    ...
    for (const dir of openings) {
      ...
      switch (dir) {
        case 0: // up
          ctx.fillRect(cx - halfPipe, y + pad, pipeWidth, size / 2 - pad - halfPipe + halfPipe);
          break;
        case 1: // right
        ...
        case 2: // down
          ...
          break;
        case 3: // left
          ctx.fillRect(x + pad, cy - halfPipe, size / 2 - pad - halfPipe + halfPipe, pipeWidth);
          break;
      }
    }
    ...
    if (pipe.connected && pipe.waterFill > 0.5 && !pipe.isSource && !pipe.isDrain) {
      ...
      ctx.fillRect(cx - halfPipe * 0.6, cy - halfPipe * 0.6, pipeWidth * 0.6, pipeWidth * 0.6);
    }
  }
