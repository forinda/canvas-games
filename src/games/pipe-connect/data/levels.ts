...
function createPipe(type: PipeType, rotation: Rotation, isSource = false, isDrain = false): Pipe {
  return {
    type,
    rotation,
    connected: isSource,
...
export function generateLevel(state: PipeState): void {
  ...
  const path = buildPath(size, state.sourceRow, state.sourceCol, state.drainRow, state.drainCol);
...
function findPipeForDirections(neededDirs: number[]): { type: PipeType; rotation: Rotation } {
