...
export class BattleRenderer implements Renderable<CardBattleState> {
  ...
  private renderCard(
    ctx: CanvasRenderingContext2D,
    ci: CardInstance, x: number, y: number,
    w: number, h: number, canPlay: boolean, selected: boolean,
  ): void {
    const card = ci.card;
  ...
  private renderOverlays(ctx: CanvasRenderingContext2D, state: CardBattleState): void {
    ...
    ctx.fillStyle = state.phase === 'win' ? '#f1c40f' : state.phase === 'round-win' ? '#2ecc71' : '#e74c3c';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';

    const title = state.phase === 'win' ? 'VICTORY!' : state.phase === 'round-win' ? 'ROUND CLEAR!' : 'DEFEATED';
