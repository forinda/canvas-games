...
export function buildLevel(level: number): PlatState {
  ...
  // Ground
  platforms.push({ x: 0, y: 500, w: 2400, h: 40, color: '#4a6741', type: 'solid' });
  ...
  for (let i = 0; i < count; i++) {
    ...
    const type: Platform['type'] = i % 5 === 0 ? 'moving' : i % 7 === 0 ? 'crumble' : 'solid';
    const p: Platform = {
      x: px,
      y: py,
      w,
      h: 16,
      color: type === 'crumble' ? '#8b5e3c' : type === 'moving' ? '#4a7ab5' : '#5a7a5a',
      type,
    };
    if (type === 'moving') {
      p.origX = px;
