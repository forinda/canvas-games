export interface WordEntry {
  word: string;
  category: string;
}

const ANIMALS: string[] = [
  'ELEPHANT', 'GIRAFFE', 'PENGUIN', 'DOLPHIN', 'CHEETAH',
  'GORILLA', 'KANGAROO', 'OCTOPUS', 'PANTHER', 'BUFFALO',
  'HAMSTER', 'LOBSTER', 'SPARROW', 'VULTURE', 'PELICAN',
  'LEOPARD', 'GAZELLE', 'PEACOCK', 'TOUCAN', 'WALRUS',
  'RACCOON', 'MUSTANG', 'FALCON', 'PARROT',
];

const COUNTRIES: string[] = [
  'BRAZIL', 'CANADA', 'FRANCE', 'GERMANY', 'JAPAN',
  'MEXICO', 'NORWAY', 'SWEDEN', 'TURKEY', 'ARGENTINA',
  'AUSTRALIA', 'COLOMBIA', 'ETHIOPIA', 'HUNGARY', 'ICELAND',
  'JAMAICA', 'PORTUGAL', 'ROMANIA', 'THAILAND', 'VIETNAM',
  'MOROCCO', 'FINLAND', 'DENMARK', 'IRELAND',
];

const FOODS: string[] = [
  'PIZZA', 'SUSHI', 'BURGER', 'WAFFLE', 'PRETZEL',
  'MUFFIN', 'COOKIE', 'NOODLE', 'PANCAKE', 'BURRITO',
  'LASAGNA', 'BROWNIE', 'BISCUIT', 'AVOCADO', 'POPCORN',
  'PUDDING', 'YOGURT', 'SALMON', 'DUMPLING', 'RAMEN',
  'OMELETTE', 'TORTILLA', 'CRACKER', 'CUSTARD',
];

const SPORTS: string[] = [
  'SOCCER', 'TENNIS', 'HOCKEY', 'BOXING', 'CRICKET',
  'CYCLING', 'FENCING', 'ROWING', 'SURFING', 'ARCHERY',
  'BASEBALL', 'FOOTBALL', 'LACROSSE', 'SWIMMING', 'HANDBALL',
  'CLIMBING', 'BOWLING', 'JAVELIN', 'MARATHON', 'SKATING',
  'TRIATHLON', 'KARATE', 'JUDO', 'DIVING',
];

const TECHNOLOGY: string[] = [
  'COMPUTER', 'INTERNET', 'KEYBOARD', 'MONITOR', 'PRINTER',
  'ROUTER', 'SERVER', 'BROWSER', 'PROGRAM', 'NETWORK',
  'DATABASE', 'FIREWALL', 'MALWARE', 'BITCOIN', 'WEBSITE',
  'DESKTOP', 'LAPTOP', 'TABLET', 'PIXEL', 'CURSOR',
  'ALGORITHM', 'COMPILER', 'SILICON', 'WIDGET',
];

const ALL_CATEGORIES: { name: string; words: string[] }[] = [
  { name: 'Animals', words: ANIMALS },
  { name: 'Countries', words: COUNTRIES },
  { name: 'Foods', words: FOODS },
  { name: 'Sports', words: SPORTS },
  { name: 'Technology', words: TECHNOLOGY },
];

export function getRandomWord(): WordEntry {
  const cat = ALL_CATEGORIES[Math.floor(Math.random() * ALL_CATEGORIES.length)];
  const word = cat.words[Math.floor(Math.random() * cat.words.length)];
  return { word, category: cat.name };
}
