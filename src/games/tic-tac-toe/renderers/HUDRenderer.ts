...
export class HUDRenderer implements Renderable<TicTacToeState> {
  ...
  private renderModeSelect(
    ...
    // AI button
    this.drawButton(ctx, centerX - btnW / 2, centerY - 10 - btnH, btnW, btnH, 'vs AI', '#ef5350');

    // 2-player button
    this.drawButton(ctx, centerX - btnW / 2, centerY + 10, btnW, btnH, '2 Players', '#42a5f5');
  ...
  private renderGameOverOverlay(
    ...
    ctx.strokeStyle = state.isDraw ? '#888' : (state.winner === 'X' ? '#ef5350' : '#42a5f5');
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.roundRect(px, py, panelW, panelH, 12);
    ctx.stroke();
