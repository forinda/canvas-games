...
export class GameRenderer implements Renderable<LavaState> {
  ...
  private drawPlatforms(ctx: CanvasRenderingContext2D, state: LavaState): void {
    for (const plat of state.platforms) {
      ...
      // Platform body
      const grad = ctx.createLinearGradient(plat.x, plat.y, plat.x, plat.y + PLATFORM_HEIGHT);
  ...
  private drawLavaBubbles(ctx: CanvasRenderingContext2D, state: LavaState): void {
