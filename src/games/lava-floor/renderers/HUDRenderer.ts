...
export class HUDRenderer implements Renderable<LavaState> {
  ...
  private drawTimer(ctx: CanvasRenderingContext2D, state: LavaState): void {
    ...
    if (state.bestTime > 0) {
      ...
      ctx.strokeStyle = '#000';
      ctx.lineWidth = 3;
      ctx.strokeText(`Best: ${state.bestTime.toFixed(1)}s`, state.canvasW - 16, 30);
      ctx.fillStyle = '#ffab40';
      ctx.fillText(`Best: ${state.bestTime.toFixed(1)}s`, state.canvasW - 16, 30);
    }
  }
  ...
  private drawIdleOverlay(
    ...
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 3;
    ctx.strokeText('Press Arrow Keys or Space to Start', cx, state.canvasH * 0.6);
    ctx.fillStyle = '#fff';
    ...
    ctx.fillStyle = '#999';
    ctx.fillText('Arrows: Move  |  Space: Jump  |  ESC: Exit', cx, state.canvasH * 0.67);
    ...
    if (state.bestTime > 0) {
      ...
      ctx.strokeStyle = '#000';
      ctx.lineWidth = 3;
      ctx.strokeText(`Best: ${state.bestTime.toFixed(1)}s`, cx, state.canvasH * 0.74);
      ctx.fillStyle = '#ffab40';
      ctx.fillText(`Best: ${state.bestTime.toFixed(1)}s`, cx, state.canvasH * 0.74);
    }
  }
