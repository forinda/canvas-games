...
import { BALL_RADIUS, RIM_THICKNESS, NET_HEIGHT, GRAVITY } from "../types";
...
export class GameRenderer implements Renderable<BasketballState> {
	...
	private drawHoop(
		...
		for (let i = 0; i <= netSegments; i++) {
			...
			const topX =
				rimLeft + (hoop.rimWidth - netWidth) / 2 + netWidth * (i / netSegments);
			const bottomX =
				hoop.x - netBottomWidth / 2 + netBottomWidth * (i / netSegments);
			...
			const midX =
				topX + (bottomX - topX) * 0.5 + Math.sin(t * Math.PI * 3) * 3;
