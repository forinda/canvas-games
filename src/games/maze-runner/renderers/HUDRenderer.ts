...
export class HUDRenderer implements Renderable<MazeState> {
  render(ctx: CanvasRenderingContext2D, state: MazeState): void {
    ...
    const timerColor = state.timeLeft <= 10 ? '#ff4444' : state.timeLeft <= 20 ? '#ffaa00' : '#4ade80';
    ...
    ctx.fillText(`Level ${state.level}  (${state.mazeW}x${state.mazeH})`, W / 2, barY);
    ...
    // Overlays
    if (!state.started) {
      this.overlay(ctx, W, H, '#607d8b', 'Maze Runner', 'Press SPACE to start  |  [H] for help');
