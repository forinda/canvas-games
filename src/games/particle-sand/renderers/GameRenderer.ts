...
export class GameRenderer implements Renderable<SandState> {
  ...
  render(ctx: CanvasRenderingContext2D, state: SandState): void {
    ...
    if (!this.imageData || this.imageData.width !== pixW || this.imageData.height !== pixH) {
    ...
    // Draw brush cursor
    if (state.mouseX >= 0 && state.mouseX < state.gridW &&
        state.mouseY >= 0 && state.mouseY < state.gridH) {
