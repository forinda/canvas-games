...
export class ParticleSystem implements Updatable<SandState> {
  ...
  private placeParticles(state: SandState): void {
    ...
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        ...
        const life = state.selectedType === 'fire' ? 60 + Math.floor(Math.random() * 80) :
                     state.selectedType === 'steam' ? 80 + Math.floor(Math.random() * 60) : 0;
  ...
  private updateSand(state: SandState, x: number, y: number, idx: number, p: Particle): void {
    // Fall straight down
  ...
  private updateWater(state: SandState, x: number, y: number, idx: number, p: Particle): void {
  ...
  private updateFire(state: SandState, x: number, y: number, idx: number, p: Particle): void {
    p.life--;
    if (p.life <= 0) {
      state.grid[idx] = null;
      return;
  ...
  private updateSteam(state: SandState, x: number, y: number, idx: number, p: Particle): void {
    p.life--;
    if (p.life <= 0) {
      state.grid[idx] = null;
      return;
