...
import { CELL_SIZE, GRID_COLS, GRID_ROWS, BRICK_COLORS, HUD_HEIGHT } from '../types';
...
export class HUDRenderer implements Renderable<BrickBuilderState> {
  ...
  private renderTopBar(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
    ...
    ctx.fillStyle = '#666';
    ctx.textAlign = 'right';
    ctx.fillText('[H] Help  [ESC] Exit  [Scroll] Rotate  [Right-click] Remove', W - 16, HUD_HEIGHT / 2);
  }

  private renderPalette(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
    const paletteX = state.gridOffsetX + GRID_COLS * CELL_SIZE + 16;
    let y = state.gridOffsetY + 8;
    ...
    for (let i = 0; i < BRICK_TEMPLATES.length; i++) {
      ...
      // Background
      ctx.fillStyle = isSelected ? 'rgba(255, 112, 67, 0.15)' : 'rgba(255, 255, 255, 0.03)';
      ctx.beginPath();
  ...
  private renderColorPicker(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
    const paletteX = state.gridOffsetX + GRID_COLS * CELL_SIZE + 16;
  ...
  private renderClearButton(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
    const paletteX = state.gridOffsetX + GRID_COLS * CELL_SIZE + 16;
  ...
  private renderBrickCount(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
    const paletteX = state.gridOffsetX + GRID_COLS * CELL_SIZE + 16;
  ...
  private renderHelp(ctx: CanvasRenderingContext2D, state: BrickBuilderState): void {
