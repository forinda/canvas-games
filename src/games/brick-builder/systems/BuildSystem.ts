...
export class BuildSystem implements Updatable<BrickBuilderState> {
  ...
  placeBrick(
    ...
    // Apply gravity: find the lowest Y where the brick can rest
    ...
    while (finalY > 0 && this.overlapsAny(state.bricks, clampedX, finalY, bw, bh)) {
      finalY--;
    }
  ...
  removeBrickAt(state: BrickBuilderState, gridX: number, gridY: number): void {
    const index = state.bricks.findIndex(
      (b) => gridX >= b.x && gridX < b.x + b.w && gridY >= b.y && gridY < b.y + b.h,
    );
