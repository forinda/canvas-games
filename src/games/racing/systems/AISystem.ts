...
import {
  ...
  AI_SPEED_FACTOR_MIN, AI_SPEED_FACTOR_MAX,
  AI_STEER_SMOOTHING, AI_WAYPOINT_RADIUS, AI_VARIATION,
  SKID_MARK_MAX,
} from '../types';
...
export class AISystem implements Updatable<RacingState> {
  ...
  initCar(car: Car): void {
    this.aiData.set(car, {
      speedFactor: AI_SPEED_FACTOR_MIN + Math.random() * (AI_SPEED_FACTOR_MAX - AI_SPEED_FACTOR_MIN),
