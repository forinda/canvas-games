...
export class HUDRenderer implements Renderable<RacingState> {
  ...
  private renderTopBar(ctx: CanvasRenderingContext2D, state: RacingState, W: number): void {
    // Background
  ...
  private renderPositions(ctx: CanvasRenderingContext2D, state: RacingState, W: number): void {
    const x = W - 140;
    const startY = 60;
