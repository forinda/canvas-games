...
export class HUDRenderer implements Renderable<CheckersState> {
  ...
  private drawTurnIndicator(ctx: CanvasRenderingContext2D, state: CheckersState, W: number): void {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(W / 2 - ctx.measureText(text).width / 2 - 18, 22, 8, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
  }
  ...
  private drawCapturedCount(
    ...
    ctx.fillStyle = '#aaa';
    ctx.fillText(`Black captured: ${state.capturedRed}`, boardX, boardY + boardSize + 30);
  }

  private drawModeLabel(ctx: CanvasRenderingContext2D, state: CheckersState, W: number): void {
  ...
  private drawAIThinking(ctx: CanvasRenderingContext2D, W: number, H: number): void {
  ...
  private drawGameOverOverlay(
    ...
    const winnerText =
      ...
      state.winner === 'black' ? 'Black Wins!' :
      'Draw!';
    const winColor = state.winner === 'red' ? '#ff4444' : state.winner === 'black' ? '#aaa' : '#fff';
    ...
    ctx.fillStyle = '#888';
    ctx.fillText(`Red: ${12 - state.capturedRed} remaining | Black: ${12 - state.capturedBlack} remaining`, W / 2, H / 2 + 15);
