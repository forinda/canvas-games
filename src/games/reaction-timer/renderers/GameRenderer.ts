...
export class GameRenderer implements Renderable<ReactionState> {
  render(ctx: CanvasRenderingContext2D, state: ReactionState): void {
    ...
    if (state.phase === 'waiting') {
      ...
      ctx.fillText(`Round ${Math.min(state.round, MAX_ROUNDS)} of ${MAX_ROUNDS}`, cx, cy + 30);
    }
    ...
    if (state.phase === 'result') {
      ...
      const label = state.reactionMs < 200 ? 'Incredible!' :
        state.reactionMs < 300 ? 'Great!' :
        state.reactionMs < 400 ? 'Good' : 'Keep trying!';
      ctx.fillText(label, cx, cy + 10);

      ctx.font = `${Math.min(18, W * 0.025)}px monospace`;
      ctx.fillStyle = 'rgba(255,255,255,0.5)';
      const nextText = state.round > MAX_ROUNDS
  ...
  private renderFinalScreen(
    ...
    ctx.fillStyle = '#888';
    ctx.font = `${Math.min(18, W * 0.025)}px monospace`;
    ctx.fillText(`All-time best: ${state.bestAllTime > 0 ? state.bestAllTime + ' ms' : 'N/A'}`, cx, cy + 15);
