...
export class HUDRenderer implements Renderable<GolfState> {
  ...
  render(ctx: CanvasRenderingContext2D, state: GolfState): void {
    ...
    if (!state.showHelp && !state.gameComplete) {
      ...
      ctx.fillText('[H] Help  [R] Restart  [ESC] Exit', W - 12, ctx.canvas.height - 8);
    }
  }
  ...
  private drawSunkOverlay(
    ...
    const alpha = Math.min(1, elapsed / 300) * (1 - Math.max(0, (elapsed - SUNK_DISPLAY_TIME + 400) / 400));
    const course = COURSES[state.currentHole];
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText(`${state.strokes} stroke${state.strokes !== 1 ? 's' : ''} - ${label}`, W / 2, H * 0.55);
  ...
  private drawCompleteOverlay(
    ...
    const finalLabel = finalDiff === 0 ? 'Even' :
      finalDiff > 0 ? `+${finalDiff}` : `${finalDiff}`;
    ctx.fillStyle = finalDiff < 0 ? '#4caf50' : finalDiff === 0 ? '#fff' : '#e53935';
