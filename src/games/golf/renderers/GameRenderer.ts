...
export class GameRenderer implements Renderable<GolfState> {
  ...
  private drawSlopes(ctx: CanvasRenderingContext2D, state: GolfState): void {
    ...
    for (let i = 0; i < course.slopes.length; i++) {
      ...
      for (let ax = slope.x + arrowSpacing; ax < slope.x + slope.width; ax += arrowSpacing) {
        for (let ay = slope.y + arrowSpacing; ay < slope.y + slope.height; ay += arrowSpacing) {
          ctx.beginPath();
          ctx.moveTo(ax, ay);
  ...
  private drawBall(ctx: CanvasRenderingContext2D, state: GolfState): void {
    ...
    ctx.beginPath();
    ctx.ellipse(ball.pos.x + 2, ball.pos.y + 3, ball.radius, ball.radius * 0.6, 0, 0, Math.PI * 2);
    ctx.fill();
