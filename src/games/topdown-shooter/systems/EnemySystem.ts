...
const TEMPLATES: Record<EnemyType, EnemyTemplate> = {
  normal: { type: 'normal', hp: 2, speed: 100, radius: 14, color: '#ef5350', damage: 10 },
  fast:   { type: 'fast',   hp: 1, speed: 190, radius: 10, color: '#ffa726', damage: 8 },
  tank:   { type: 'tank',   hp: 6, speed: 55,  radius: 22, color: '#8d6e63', damage: 20 },
  ranged: { type: 'ranged', hp: 2, speed: 60,  radius: 13, color: '#ab47bc', damage: 8 },
};
...
export class EnemySystem implements Updatable<ShooterState> {
  ...
  private randomEdgePosition(w: number, h: number) {
    ...
    switch (side) {
      case 0: return { x: pad, y: pad + Math.random() * (h - 2 * pad) };           // left
      case 1: return { x: w - pad, y: pad + Math.random() * (h - 2 * pad) };       // right
      case 2: return { x: pad + Math.random() * (w - 2 * pad), y: pad };            // top
      default: return { x: pad + Math.random() * (w - 2 * pad), y: h - pad };       // bottom
    }
  }
}
