  ...
  update(state: ShooterState, dt: number): void {
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        b.pos.x < ARENA_PADDING || b.pos.x > state.canvasW - ARENA_PADDING ||
        b.pos.y < ARENA_PADDING || b.pos.y > state.canvasH - ARENA_PADDING
      ) {
        bullets.splice(i, 1);
        continue;
      }
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
          ...
          if (dist < b.radius + e.radius) {
            ...
            if (e.hp <= 0) {
              ...
              state.score += e.type === 'tank' ? 30 : e.type === 'ranged' ? 20 : e.type === 'fast' ? 15 : 10;
              state.kills += 1;
              enemies.splice(j, 1);
            }
