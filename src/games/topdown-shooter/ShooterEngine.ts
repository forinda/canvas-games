...
export class ShooterEngine {
  ...
  private loop(): void {
    ...
    if (this.state.started && !this.state.paused && !this.state.gameOver && !this.hudRenderer.helpVisible) {
  ...
  private createInitialState(w: number, h: number, highScore: number): ShooterState {
    return {
      canvasW: w,
      canvasH: h,
      player: {
