...
export class HUDRenderer implements Renderable<ShooterState> {
  ...
  render(ctx: CanvasRenderingContext2D, state: ShooterState): void {
    ...
    const hpColor = hpFrac > 0.5 ? '#4caf50' : hpFrac > 0.25 ? '#ff9800' : '#f44336';
    ...
    ctx.fillText(`${state.player.hp} / ${state.player.maxHp}`, barX + barW / 2, barY + barH / 2);
    ...
    if (!state.started) {
      ...
      ctx.fillStyle = '#ccc';
      ctx.fillText('Click or press Space to start', W / 2, state.canvasH / 2 + 10);
      ctx.font = '14px monospace';
      ctx.fillStyle = '#888';
      ctx.fillText('WASD to move, Mouse to aim, Click to shoot', W / 2, state.canvasH / 2 + 45);
    }
    ...
    if (state.gameOver) {
      ...
      ctx.fillStyle = '#fff';
      ctx.fillText(`Score: ${state.score}  |  Wave: ${state.waveData.wave}  |  Kills: ${state.kills}`, W / 2, state.canvasH / 2);
      ...
      ctx.font = '16px monospace';
      ctx.fillStyle = '#aaa';
      ctx.fillText('Click or press Space to restart', W / 2, state.canvasH / 2 + 65);
    }
