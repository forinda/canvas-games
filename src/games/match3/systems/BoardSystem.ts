...
import { GEM_TYPES, ROWS, COLS, FALL_SPEED, REMOVE_DURATION, SWAP_DURATION } from '../types';
...
export class BoardSystem implements Updatable<Match3State> {
  ...
  swap(state: Match3State, rA: number, cA: number, rB: number, cB: number): void {
    const a = state.board[rA][cA];
    const b = state.board[rB][cB];
    if (a) { a.row = rB; a.col = cB; }
    if (b) { b.row = rA; b.col = cA; }
    state.board[rA][cA] = b;
    state.board[rB][cB] = a;
  }
    ...
    if (state.phaseTimer >= SWAP_DURATION) {
      ...
      if (matches.size > 0) {
        ...
        if (state.swapA && state.swapB) {
          this.swap(state, state.swapA.row, state.swapA.col, state.swapB.row, state.swapB.col);
        }
