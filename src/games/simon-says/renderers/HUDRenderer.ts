...
export class HUDRenderer implements Renderable<SimonState> {
  ...
  private drawStartOverlay(ctx: CanvasRenderingContext2D, state: SimonState): void {
    const W = state.canvasW;
  ...
  private drawGameOverOverlay(ctx: CanvasRenderingContext2D, state: SimonState): void {
    const W = state.canvasW;
    const H = state.canvasH;
