...
export class HUDRenderer implements Renderable<FroggerState> {
  render(ctx: CanvasRenderingContext2D, state: FroggerState): void {
    ...
    if (!state.started) {
      this.drawOverlay(ctx, W, H, '🐸 FROGGER', 'Press any arrow key or WASD to start', '#4caf50');
      return;
    }
    ...
    if (state.gameOver) {
      this.drawOverlay(ctx, W, H, 'GAME OVER', `Score: ${state.score}  |  Press Space to restart`, '#f44336');
      return;
    }
  }
