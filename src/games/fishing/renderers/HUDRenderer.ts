...
export class HUDRenderer implements Renderable<FishingState> {
  ...
  private drawPowerMeter(
    ...
    const displayPower = state.castPower <= 1 ? state.castPower : 2 - state.castPower;
  ...
  private drawTensionMeter(
    ...
    ctx.fillStyle = state.reelTension > 0.3 && state.reelTension < 0.7 ? '#4caf50' : '#f44336';
    ctx.beginPath();
  ...
  private drawCatchPopup(
    ...
    ctx.font = '14px monospace';
    ctx.fillStyle = '#ccc';
    ctx.fillText(`${caught.size} cm  •  ${caught.fish.rarity.toUpperCase()}`, W / 2, py + 70);
