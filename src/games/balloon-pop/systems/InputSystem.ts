...
export class InputSystem implements InputHandler {
  ...
  private handleKey(e: KeyboardEvent): void {
    ...
    if (e.key === ' ' || e.key === 'Enter') {
      ...
      if (s.phase === 'gameover') { this.onReset(); return; }
    }
  }
  ...
  private processPointer(clientX: number, clientY: number): void {
    ...
    if (mx < 80 && my < 40) { this.onExit(); return; }
    ...
    for (let i = s.balloons.length - 1; i >= 0; i--) {
      ...
      if (dx * dx + dy * dy <= b.radius * b.radius) {
        ...
        if (s.score > s.highScore) {
          s.highScore = s.score;
          try { localStorage.setItem(HS_KEY, String(s.highScore)); } catch { /* noop */ }
        }
  ...
  private spawnPopParticles(x: number, y: number, color: string, radius: number): void {
