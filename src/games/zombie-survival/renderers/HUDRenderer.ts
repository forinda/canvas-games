...
export class HUDRenderer implements Renderable<GameState> {
	...
	private drawTopBar(
		...
		ctx.fillText(
			`Ammo: ${Math.floor(state.player.ammo)}/${state.player.maxAmmo}`,
			x,
			y,
		);
		x += 140;
	...
	private drawWaveInfo(
		...
		ctx.fillText(
			`${phaseLabel} - Wave ${state.wave} - ${timeLeft}s`,
			W - 12,
			18,
		);
		...
		if (isNight && state.zombiesRemainingInWave > 0) {
			...
			ctx.fillText(
				`Zombies remaining: ${state.zombiesRemainingInWave}`,
				W - 12,
				34,
			);
		}
	...
	private drawGameOver(
		...
		ctx.fillStyle = "#f1c40f";
		ctx.fillText(
			`Score: ${state.score}  |  Kills: ${state.totalKills}`,
			W / 2,
			H / 2 + 25,
		);
