...
export class GhostSystem implements Updatable<PacManState> {
	...
	private updateModeTimers(state: PacManState, dt: number): void {
		...
		if (state.modeTimer >= duration) {
			state.modeTimer = 0;
			state.modeIndex = Math.min(
				state.modeIndex + 1,
				MODE_DURATIONS.length - 1,
			);
	...
	private moveGhost(ghost: Ghost, state: PacManState, dt: number): void {
		...
		const distToCenter =
			Math.abs(ghost.pos.x - cx) + Math.abs(ghost.pos.y - cy);
		...
		if (distToCenter < 0.15) {
			...
			if (ghost.eaten) {
				if (
					Math.abs(cx - ghost.homePos.x) < 1 &&
					Math.abs(cy - ghost.homePos.y) < 1
				) {
					ghost.eaten = false;
					ghost.mode =
						state.frightenedTimer > 0 ? "frightened" : state.globalMode;
				}
			}
