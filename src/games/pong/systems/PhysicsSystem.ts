...
export class PhysicsSystem implements Updatable<PongState> {
  ...
  private movePaddles(s: PongState, dt: number): void {
    ...
    s.leftPaddle.y = Math.max(minY, Math.min(maxY, s.leftPaddle.y + s.leftPaddle.dy * dt));
    s.rightPaddle.y = Math.max(minY, Math.min(maxY, s.rightPaddle.y + s.rightPaddle.dy * dt));
  }
