...
export class InputSystem implements InputHandler {
  ...
  constructor(
    ...
    this.keyHandler = (e) => {
      ...
      if (num >= 1 && num <= 6) this.state.selectedType = BUILDING_TYPES[num - 1];
      if (e.key === '+' || e.key === '=') this.state.speed = Math.min(3, this.state.speed + 1);
  ...
  private handleClick(e: MouseEvent): void {
    ...
    s.money -= def.cost;
    this.gridSystem.placeBuilding(s, { type: s.selectedType, col, row, level: 1 });
