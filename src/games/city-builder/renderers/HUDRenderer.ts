...
export class HUDRenderer implements Renderable<CityState> {
  ...
  render(ctx: CanvasRenderingContext2D, state: CityState): void {
    ...
    const stats: [string, string, string][] = [
      ...
      ['\u{1F60A}', `${s.happiness}%`, s.happiness > 60 ? '#2ecc71' : s.happiness > 30 ? '#f39c12' : '#e74c3c'],
    ...
    // Start overlay
    if (!s.started) {
      this.drawOverlay(ctx, W, H, 'CITY BUILDER', 'Select a building from the panel, click the grid to place.\nManage population, food, power, and happiness!', '#3498db');
    }
  }
