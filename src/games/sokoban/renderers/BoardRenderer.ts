...
export class BoardRenderer implements Renderable<SokobanState> {
  ...
  private drawWall(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
    const inset = 1;
    // Bottom (shadow)
  ...
  private drawFloor(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
  ...
  private drawTarget(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
    const cx = x + size / 2;
    const cy = y + size / 2;
    const r = size * 0.2;
  ...
  private drawPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
    const cx = x + size / 2;
    const cy = y + size / 2;
    const r = size * 0.35;
