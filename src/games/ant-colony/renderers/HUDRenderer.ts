...
export class HUDRenderer implements Renderable<AntColonyState> {
  ...
  private _drawStatsPanel(ctx: CanvasRenderingContext2D, state: AntColonyState): void {
    const x = 12;
    let y = 16;
    const lh = 18;
  ...
  private _drawTaskBars(ctx: CanvasRenderingContext2D, state: AntColonyState): void {
    const x = 12;
    const y = 114;
    const barW = 150;
    const barH = 8;
    ...
    const tasks: { label: string; key: string; value: number; color: string }[] = [
