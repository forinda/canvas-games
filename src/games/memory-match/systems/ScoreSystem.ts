...
export class ScoreSystem implements Updatable<MemoryState> {
  update(state: MemoryState, dt: number): void {
    if (state.timerRunning && state.started && !state.paused && !state.gameOver) {
  ...
  private persistBest(state: MemoryState): void {
    const key = LS_PREFIX + state.difficulty;
    const isBetterMoves = state.bestMoves === null || state.moves < state.bestMoves;
    const isBetterTime = state.bestTime === null || state.elapsedTime < state.bestTime;
    ...
    if (isBetterMoves || isBetterTime) {
      ...
      try {
        localStorage.setItem(key, JSON.stringify({ moves: newMoves, time: newTime }));
      } catch {
        /* storage unavailable */
      }
    }
