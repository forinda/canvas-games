...
export class HUDRenderer implements Renderable<MemoryState> {
  render(ctx: CanvasRenderingContext2D, state: MemoryState): void {
    ...
    if (state.bestMoves !== null || state.bestTime !== null) {
      ...
      if (state.bestMoves !== null) bestParts.push(`Best: ${state.bestMoves} moves`);
      if (state.bestTime !== null) bestParts.push(this.formatTime(state.bestTime));
  ...
  private drawStartOverlay(ctx: CanvasRenderingContext2D, state: MemoryState): void {
    const W = state.canvasW;
  ...
  private drawWinOverlay(ctx: CanvasRenderingContext2D, state: MemoryState): void {
    const W = state.canvasW;
    const H = state.canvasH;
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText(`${state.moves} moves  |  ${this.formatTime(state.elapsedTime)}`, W / 2, py + 80);
    ...
    ctx.font = '14px monospace';
    ctx.fillStyle = '#888';
    ctx.fillText('[Space] Play again  |  \u2190\u2192 Change difficulty', W / 2, py + 150);
