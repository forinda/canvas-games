  ...
  render(ctx: CanvasRenderingContext2D, state: WhackState): void {
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.ellipse(cx, cy + holeRadiusY * 0.5, holeRadiusX + 2, holeRadiusY + 2, 0, 0, Math.PI * 2);
        ctx.stroke();
        ...
        if (hole.state !== 'empty') {
          ...
          ctx.save();
          ctx.beginPath();
          ctx.rect(cx - holeRadiusX, 0, holeRadiusX * 2, cy + holeRadiusY * 0.5);
          ctx.clip();
          ...
          if (hole.isBomb) {
            ...
            ctx.quadraticCurveTo(cx + 8, moleY - moleRadius - 12, cx + 4, moleY - moleRadius - 8);
            ctx.stroke();
            ...
            ctx.beginPath();
            ctx.arc(cx, moleY + moleRadius * 0.15, moleRadius * 0.6, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.arc(cx, moleY + moleRadius * 0.1, moleRadius * 0.12, 0, Math.PI * 2);
            ctx.fill();
          }
