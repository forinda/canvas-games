...
export class PlatformMenu {
	...
	private registry: Record<GameCategory, GameDefinition[]> = {} as Record<
		GameCategory,
		GameDefinition[]
	>;
	...
	show(
		registry: Record<GameCategory, GameDefinition[]>,
	...
	private render(): void {
		...
		ctx.fillText(
			`${filtered.length} of ${this.allGames.length} games`,
			W / 2,
			52,
		);
		...
		tabs.forEach((tab, i) => {
			...
			const count =
				cat.key === "all"
					...
					: (this.registry[cat.key]?.length ?? 0);
	...
	private renderCard(
		...
		ctx.textAlign = "center";
		ctx.textBaseline = "middle";
		ctx.fillText(game.icon, x + w / 2, y + h * 0.3);
		...
		ctx.beginPath();
		ctx.roundRect(btnX, btnY, btnW2, btnH2, 5);
		ctx.fill();
